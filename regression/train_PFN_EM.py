...
parser.add_argument('--EM_info', action="store_true", dest="em_info")
parser.add_argument('--no_EM_info', action="store_false", dest="em_info")
parser.set_defaults(em_info=False)
...
if args.hard_code:
    ...
    print('Including EM info? {}'.format(EM_INFO))
    if args.full_data:
...
if args.full_data:
    if EM_INFO == True:
        X = np.load(datapath_prefix+'X_PIPM_full_norm.npy',
                    mmap_mode='r+')[:,:,:5]
        Y = np.load(datapath_prefix+'Y_PIPM_full_norm.npy', mmap_mode='r+')
    else:
        X = np.load(datapath_prefix+'X_PIPM_full_norm.npy',
                    mmap_mode='r+')[:,:,:4]
        Y = np.load(datapath_prefix+'Y_PIPM_full_norm.npy', mmap_mode='r+')
    ...
    if EM_INFO == True:
        X = np.load(datapath_prefix+'X_PIPM_full_norm.npy',
                    mmap_mode='r+')[:NEVENTS,:,:5]
        Y = np.load(datapath_prefix+'Y_PIPM_full_norm.npy',
                    mmap_mode='r+')[:NEVENTS]
    else:
        X = np.load(datapath_prefix+'X_PIPM_full_norm.npy',
                    mmap_mode='r+')[:NEVENTS,:,:4]
        Y = np.load(datapath_prefix+'Y_PIPM_full_norm.npy',
                    mmap_mode='r+')[:NEVENTS]
t1 = cput()
...
#======================================
if EM_INFO == True:
    infostring = '{}_wEM_PIPM--LR_{:.0e}--BS_{}--EP_{}--EV_{}--{}'.format(MODEL,
                LEARNING_RATE, BATCH_SIZE, EPOCHS, NEVENTS, DATE)
else:
    infostring = '{}_noEM_PIPM--LR_{:.0e}--BS_{}--EP_{}--EV_{}--{}'.format(MODEL,
                LEARNING_RATE, BATCH_SIZE, EPOCHS, NEVENTS, DATE)
