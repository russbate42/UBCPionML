...
for currFile in fileNames:
    ...
    if firstArray:
        ...
        Y = np.ndarray.copy(Y_new)
        old_max_dims = max_dims
        firstArray = False
    else:
        Y = np.concatenate((Y, Y_new))
        
        if max_dims[1] > old_max_dims[1]:
            # need the size of all events not including the new file
            old_size = tot_nEvts - max_dims[0]
            # use new size to match axis 1 with old values, zero pad old values
            X_padded = np.zeros((old_size,max_dims[1],6))
            X_padded[:old_size,:old_max_dims[1],:6] = X[:old_size,:old_max_dims[1],:6]
            # update the max dimensions
            old_max_dims = max_dims
            X = np.concatenate((X_padded, X_new), axis=0)
            
        elif max_dims[1] == old_max_dims[1]:
            # sizes are the same, easy concatenation
            X = np.concatenate((X, X_new), axis=0)
        
        else:
            # use old size to match axis 1 with new values, zero pad new values
            X_padded = np.zeros((max_dims[0], old_max_dims[1], 6))
            X_padded[:max_dims[0], :max_dims[1], :6] = X_new
            X = np.concatenate((X, X_padded), axis=0)
    t1 = t.time()
    time_to_concatenate = t1-t0
    t_tot += indices_time+array_construction_time+time_to_concatenate
    
    print('Array dimension: '+str(current_dims))
    print('Time to construct index array: '+str(indices_time))
    print('Time to populate elements: '+str(array_construction_time))
    print('Time to concatenate arrays: '+str(time_to_concatenate))
    print('Time for this file: '+str(indices_time+array_construction_time+time_to_concatenate))
    print('Total events: '+str(tot_nEvts))
    print('Current size: '+str(X.shape))
    print('Total time: '+str(t_tot))
    print()
    
t0 = t.time()
np.savez('/fast_scratch/atlas_images/v01-45/XY_tracks_multipleclusters_allfiles', X, Y)
t1 = t.time()
print()
