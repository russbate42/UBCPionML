...
plabels = np_utils.to_categorical(pdata_merged['label'],
...
#################################################
t_tot0 = t.time() # time this process 
...
print('..running layer annealing..');print()
N = 10 #number of temperature steps
M = 3 #number of sweeps per temperature
...
def layer_size(current_size, scale_factor):
    ...
    A = (-current_size - mean)/sigma
    B = (current_size - mean)/sigma
    ## Truncnorm distribution test
    delta = truncnorm.rvs(a=A, b=B, loc=0,
            scale=sigma, size=1)
...
sigma_layer = np.linspace(.3,.025,num=N)
...
for layer in layers:
    ...
    data_str = datapath
    data_str += 'Simple_MC_Optimization/'+layer+'_nodes.txt'
    tf = open(data_str, 'w')
    tf.write('#############################\n')
    tf.write('## '+layer+' ROC CURVE ######\n')
    tf.write('#############################\n')
    tf.write('## 100 epochs, 200 batch size, 2 GPU\n')
    tf.write('## Random seed: 648967\n')
    tf.write('## LR: 5e-5, DR: .2\n')
    tf.write('## Dense1  Dense2  Dense3  Area_Under_ROC\n\n')
    ...
    LR, DR = 5e-5, .2
    old_auc = 0
    old_layers = np.array([npix, npix, np.rint(npix/2)])
    new_layers = old_layers
    
    for i in range(N):
        print('  temp step '+str(i))    
        for j in range(M):
            ...
            current_model = ntm.baseline_nn_model(strategy,
                            number_pixels=npix, l_rate=LR,
                            dropout_rate=DR, layersize=new_layers)           
            ...
            file_str = str(new_layers[0]) + '  ' + str(new_layers[1]) + '  '
            file_str += str(new_layers[2]) + '  ' + str(current_auc)
            tf.write(file_str)
            ...
            ## Choose new layer size
            for k in range(len(new_layers)):
                cl_size = old_layers[k]
                delta_node = layer_size(current_size=cl_size,
                    scale_factor=sigma_layer[k])
                new_layers[k] = cl_size + delta_node

    tf.close()

t_tot1 = t.time()
print('Total time to run annealing on all layers: '+\
    str(t_tot1-t_tot0)+' (s)'); print()
